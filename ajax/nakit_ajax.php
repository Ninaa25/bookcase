<?php
    ...
    if(isset($_POST['korisnicko_ime_nakit']) && isset($_POST['komentar_nakit']) && isset($_POST['ocena_nakit'])){
    $values = array($_POST["korisnicko_ime_nakit"],$_POST["komentar_nakit"],$_POST["ocena_nakit"]);
    $crud=new Database();
    $crud->insert("nakit",$values);
    }

    if(isset($_POST['delete_id'])){
        $id=$_POST['delete_id'];
        $crud=new Database();
        $crud->delete("nakit",$id);
    }
    ...
    if(isset($_POST['komentar']) && isset($_POST['ocena'])){
        ...
        $crud->update("nakit",$values);
    }
    
    if(isset($_POST['search_post_btn'])){
        $ime=$_POST['ime'];
        $crud=new Database();
        $query_run=$crud->select("nakit","id,korisnicko_ime,komentar,ocena","korisnicko_ime='$ime'");
   ...
   if(isset($_POST['btn'])){
    $crud=new Database();
    $query_run=$crud->select("nakit","id,korisnicko_ime,komentar,ocena",null,"ocena DESC");
    ?>
    <thead>
    <tr>
      <th scope="col"> <h3>Ime</h3> </th>
